...
class User(UserMixin, db.Model):
    ...
    comments = relationship(
        'Comment', back_populates="author", lazy='subquery', cascade='all, delete')
...
def edit_user(user_id):
    user = db.get_or_404(
        User, user_id, description='Sorry! No user with this id found in the database')
...
def delete_user(user_id):
    user = db.get_or_404(
        User, user_id, description='Sorry! No article with this id found in the database')
...
def get_my_posts():
    ...
    name = current_user.name
    posts = db.session.execute(db.select(BlogPost).where(
        BlogPost.author_id == user_id)).scalars()
    all_posts = posts
...
def get_post(post_id):
    post = db.get_or_404(
        BlogPost, post_id, description='Sorry!! There is no post with this ID in our database.')
    name = current_user.name
    form = CommentForm()
    if form.validate_on_submit():
...
def edit_post(post_id):
    post = db.get_or_404(
        BlogPost, post_id, description='Sorry! No article with this id found in the database')
